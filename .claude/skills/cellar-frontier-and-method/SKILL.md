...
### (1) Ship user notifications — the flagship unshipped ambition

**Why it's open.** The public changelog promised this to users twice — `packages/frontend/src/pages/Updates/data.ts:44-52` (Nov 2024, two "Phase 1" entries) and `:29-32` (Feb 2025, "Better Notification Settings (Phase 2)", *"You'll soon be able to receive notifications about your favorite comedians…"*) — and the one attempt, PR #44 "wip new comic email", died closed-unmerged (see `cellar-failure-archaeology` Entry 10). **No user email has ever been sent.** Every email path goes admin-to-self: `packages/core/email.ts:22-23` sets `from: FromEmail, to: FromEmail` — verified 2026-07-07. No code anywhere reads the notification tables to send anything.

**The asset already in place** (so this is wiring, not green-field):
...
- Two crons that already run on schedule and already `sendEmail` — `packages/functions/cron/newShowCron.ts`, `syncCron.ts` (wired in `infra/cron.ts`).
- nodemailer plumbing — `packages/core/email.ts` (the `to:` is the only thing between here and a real send).

**First three steps (file-level):**
1. Write the trigger query as a **pure function in `packages/core`** so it is unit-testable with zero mocks: "acts newly linked to a show, joined to `comic_notification` rows with `enabled = true`, that have not yet been notified." Keep it a query-returning function, not a side-effecting one. (Pure-function testing pattern: `cellar-validation-and-qa` §3a/§5.)
2. Add a **sent-ledger for idempotency** — a schema addition through the `cellar-data-model` migration workflow (`pnpm db generate` → review → `pnpm db migrate`, owner-gated). This is required, not optional: `show_notification` has a unique index on `userId` **alone** (`user_show_unique`, `showNotification.sql.ts:33`), so there is no per-show dedup to lean on; a "(user, show/act) already sent" ledger is the only thing that makes re-runs safe.
3. Wire the send into `newShowCron` **behind a NEW env flag**, dev-stage only, sending to the **admin address first** as a shadow launch (reuse `email.ts` but keep `to: FromEmail` until the shadow proves out). Adding an env flag is a config axis — follow the add-one checklist in `cellar-config-and-secrets` (mirror the existing `IS_ACTIVE`/`IS_CRON` pattern at `newShowCron.ts:10-16`), do not hardcode a stage check.

**Milestone (you have a result when):** a flagged dev-stage run emits **exactly the predicted N** notifications for a seeded scenario, and a second identical run emits **ZERO** (the ledger holds — proven idempotent). Predict N before running (Part 2).

**Promotion.** Turning the shadow launch into real outbound email to real users is a product + consent change: **owner decision + `cellar-change-control`** (§5 explicitly gates "anything that sends email/Slack/notifications to anyone other than the owner"). Do not flip `to:` off the admin address on any stage without that.

### (2) Scraper self-monitoring

**Why it's open.** The defining incident (Sep-2024 anti-bot breakage) went undetected for up to 5 months because nobody was watching and there is no monitoring — the only failure signal is an admin self-email that only fires on an *exception*, not on *silent staleness* (`syncCron.ts:90-97`). See `cellar-failure-archaeology` Entry 1 and Entry 4.
...
**Why it's open.** There is **zero** backend test coverage today; the only automation is the frontend CI trio, and it has never even gone green (`cellar-validation-and-qa` §1). The *how-to* — vitest at the root workspace, the alias config, the first pure-function targets — is fully specified in `cellar-validation-and-qa` §5 as a CANDIDATE plan. **This entry states only the frontier goal; do not duplicate that plan here.**
...
|---|---|---|
| N-plicated shows + wrong totals | `getShows`/`getShowsCount` `innerJoin(act)` then `innerJoin(comic)` with **no GROUP BY / DISTINCT** — `packages/core/models/show.ts:207-218` and `:235-241` | A show with N comics appears **N times** in `/api/shows/new`; `total` counts act-rows, not shows |
...
|---|---|
| Notifications still unshipped (no user send path) | `grep -rn "to:" packages/core/email.ts` (expect `to: FromEmail`); confirm crons/settings only touch tables, don't send |
...
| `getShows` still N-plicates (no GROUP BY/DISTINCT) | `sed -n '207,246p' packages/core/models/show.ts` (expect `innerJoin(act)`, no `groupBy`) |
