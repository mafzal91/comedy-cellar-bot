...
### 1.2 SST resource graph (as of 2026-07-13, HEAD 7ab8ac7 + working tree; same resource set since 2026-07-07/c8d9918 except the third cron; #65 swapped Gmail/nodemailer for the SES `Email` identity, then a working-tree edit made its creation stage-conditional)
...
| `sst.aws.Email "Email"` | infra/email.ts:3-9 | non-secret domain identity `mail.comedycellar.mafz.al`; the SES sender for all mail. Create-vs-adopt is stage-gated (2026-07-13, working tree): only stage `mohammadafzal` runs `new sst.aws.Email` (creates the identity + Cloudflare DKIM DNS); every other stage — prod included — adopts the existing account-level identity via `sst.aws.Email.get("Email", "mail.comedycellar.mafz.al")` (SES domain identities are account singletons; a second stage creating the same domain collides). Linked into all three crons + the `/sync-shows` and reservation routes (grants each Lambda SES IAM). See weak point 13 |
...
| 13 | SES identity ownership inversion: the DEV stage `mohammadafzal` creates and manages the account-singleton `Email` identity; prod only adopts it via `sst.aws.Email.get`. Combined with weak point 5's removal policy (`mohammadafzal` ≠ `"production"` → policy "remove"), `sst remove --stage=mohammadafzal` would delete the identity + DKIM DNS that PROD sends through. Deploying prod into a fresh AWS account before the identity exists also fails at `.get()` | infra/email.ts:3-9; sst.config.ts:8 | open, latent (working-tree change, 2026-07-13) |
...
Reconciled 2026-07-13 (second pass) against the WORKING TREE — uncommitted change after `7ab8ac7` (#65, the SES migration): infra/email.ts made stage-conditional (`mohammadafzal` creates the Email identity, every other stage adopts via `sst.aws.Email.get`); added weak point 13. Root sst-env.d.ts regenerated in the same tree (key reordering only, no type change). If this diff never lands, revert row `sst.aws.Email` in §1.2 and weak point 13 to the unconditional-create description.
