...
| 9 | Frontend CI/build fails | Does CI red on `tsc` at `@clerk/types` (TS2307)? | CI-red `@clerk/types` in frontend-only install (local "passes" = root-hoist phantom) vs `ERR_PNPM_OUTDATED_LOCKFILE` drift | Undeclared `@clerk/types` — the CURRENT CI-red cause (cellar-validation-and-qa §1); OR nested-workspace/lockfile trap |
...
**Current CI-red cause (as of 2026-07-07): undeclared `@clerk/types`.** Frontend CI has NEVER passed on main. `pnpm exec tsc --noEmit` fails with `src/hooks/useAuth.ts(2,30): error TS2307: Cannot find module '@clerk/types'` (CI order: install → eslint passes → tsc FAILS → build skipped). `@clerk/types` is imported at useAuth.ts:2 but is NOT a declared dependency in packages/frontend/package.json (only `@clerk/clerk-js` is) — a phantom dependency. CI runs `pnpm install --frozen-lockfile` INSIDE `packages/frontend`, so the type package is unresolvable there and tsc reds. It resolves only LOCALLY, when a prior repo-root `pnpm install` hoisted it where a directory walk-up finds it. **cellar-validation-and-qa §1 owns this fact and the candidate fix (add `@clerk/types` to packages/frontend) — do not re-derive or apply it here.**

Consequence for the local gate: the sequence below is CI-faithful ONLY on a tree where just `packages/frontend` deps are installed (no repo-root hoist) — there it reds at tsc on `@clerk/types`, matching CI. Where a root install has hoisted the phantom it "passes", and that passing is itself the phantom-dep symptom, not a green pipeline. Never claim "typecheck passes in CI" from a local pass.
