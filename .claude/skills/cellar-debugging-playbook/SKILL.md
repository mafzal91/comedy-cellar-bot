...
| 9 | Frontend CI/build fails | Any `tsc` error? Still TS2307 on `@clerk/types`? | Reverted/missing #63 `@clerk/types` declaration vs `ERR_PNPM_OUTDATED_LOCKFILE` drift vs stale `dist/` contaminating local `tsc` | Since #63 (merged 2026-07-12) the frontend trio is GREEN — a fresh TS2307 means the declaration was reverted; else a nested-workspace/lockfile trap (cellar-validation-and-qa §1) |
...
Three crons exist (infra/cron.ts): `Cron` → newShowCron every 6h (`cron(0 0/6 * * ? *)`, line 13), `SyncCron` → syncCron hourly (line 26), and `ShowNotificationCron` → showNotificationCron every 15 min (`cron(0/15 * * * ? *)`, line 39, added #62 2026-07-13 — this is the one that emails opted-in users about new shows; see Symptom 12). All three deploy in ALL stages but set `IS_ACTIVE = ($app.stage === "prod" ? "1" : "0")` (infra/cron.ts:9,22,35); handlers no-op via `if (!IS_ACTIVE && IS_CRON) return;` (newShowCron.ts:14-16, syncCron.ts:45-47, showNotificationCron.ts:21-22). **So on a dev stage, scheduled crons silently do nothing — by design** (added 70baa86, 2024-10-28, after a new cron initially ran in all stages). The same syncCron code IS runnable in any stage via `GET /sync-shows` because the HTTP route sets no env vars (infra/api.ts:37-40) — IS_CRON is unset, guard passes.
...
**Status (as of 2026-07-13): the frontend trio (eslint → tsc → build) is GREEN.** It was red on EVERY run until PR #63 (merged 2026-07-12, first green run on head 1fea669) declared the previously-phantom `@clerk/types` in packages/frontend/package.json (`"@clerk/types": "^4.101.25"`, line ~11). Before #63, `pnpm exec tsc --noEmit` failed with `src/hooks/useAuth.ts(2,30): error TS2307: Cannot find module '@clerk/types'` (CI order: install → eslint passes → tsc FAILS → build skipped): `@clerk/types` is imported at useAuth.ts:2 but was NOT declared (only `@clerk/clerk-js` was), and CI runs `pnpm install --frozen-lockfile` INSIDE `packages/frontend` where the undeclared package was unresolvable — it resolved only LOCALLY, when a prior repo-root `pnpm install` hoisted it where a directory walk-up found it. **If you STILL see that TS2307 today, the #63 declaration was reverted or removed — re-add it; otherwise the phantom-dep failure is fixed.** cellar-validation-and-qa §1 owns this fact and the fix history — do not re-derive it here.

Consequence for the local gate: run the sequence below with ONLY `packages/frontend` deps installed (no repo-root hoist) to match what CI actually does. A local pass is still not proof of a CI pass — a repo-root install can hoist packages that a frontend-only `--frozen-lockfile` install would not resolve, and stale `dist/` artifacts can contaminate a local `tsc`. Confirm green in the real frontend-ci.yml run, not from a local invocation.
...
Reconciled 2026-07-13 against commit 5ceaf98 (new main) for two drifts: frontend CI went GREEN (PR #63 declared `@clerk/types`, merged 2026-07-12) and SHOW notifications now ship to opted-in users (PR #62 — `new_show_queue` outbox / migration 0003, `ShowNotificationCron`, `sendHtmlEmail`, react-email template). Updated Symptoms 2, 9, 12, the triage row 9, and the re-verify table accordingly by re-reading infra/cron.ts, packages/core/email.ts, packages/frontend/package.json, packages/functions/cron/showNotificationCron.ts, packages/core/models/showNotification.ts, and migrations/. COMIC notifications remain UNSHIPPED (nothing reads `comic_notification`); backend still has no CI and zero automated tests; the show-notification path is shipped-but-unproven.

Verified 2026-07-07 against the working tree at commit c8d9918 (branch claude/skill-library-continuity-4m3x56 == main) by reading: packages/core/{requester,parseLineUp,fetchLineUp,handleLineUp,handleShowDetails,handleShowList,getFutureDatesByDay,utils,email,createReservation}.ts, packages/core/models/{show,comic,room,act,comicNotification}.ts, packages/core/sql/{show,room,comic}.sql.ts, packages/functions/cron/{newShowCron,syncCron}.ts, packages/functions/{shows,settings}/index.ts, packages/functions/webhooks/clerk.ts, infra/{cron,api,config}.ts, packages/frontend/src/utils/api.ts, packages/frontend/src/pages/Comics/index.tsx, .github/workflows/frontend-ci.yml, package.json. Commands run: the Symptom-5 `node -e` proof (output as shown); `git show` on 953e0ef, 6c3da50, 64b3ee2 and existence checks on ebc8f5a a9a902b 42ab4ac b697c94 7ea6a4b 9ce39a1 2fd6b30 6192496 c8d9918 adafd66 70baa86 f8b6976 a9e371c d020632 6bf5945 dee1ae7 f95f295 741ca41; the Symptom-9 frontend gate sequence (as of 2026-07-07 it passed LOCALLY only, via the root-hoisted `@clerk/types` phantom — NOT CI-faithful; CI was red at tsc on `@clerk/types` then, since FIXED by #63 — see the reconciliation note above and cellar-validation-and-qa §1). Not locally verifiable (pre-graft, recovered from GitHub history by the discovery pass): 6886326 (Etc/GMT+5 DST fix), 122ccf5 (Sep-2024 header recovery).

| Volatile fact | Re-verify with |
|---|---|
| Cron schedules (6h / hourly / 15-min) and IS_ACTIVE gating — expect THREE crons (Cron, SyncCron, ShowNotificationCron) | `sed -n '1,40p' infra/cron.ts` |
